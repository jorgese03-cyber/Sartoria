...
export default function RegisterPage() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (errorMessage.includes('weak_password') || errorMessage.includes('at least')) {
                errorMessage = t('auth.errors.weak_password', 'La contraseña debe tener al menos 6 caracteres.');
            } else if (errorMessage.includes('valid email')) {
                errorMessage = t('auth.errors.invalid_email', 'Por favor introduce un email válido.');
            } else if (errorMessage.includes('Anonymous sign-ins are disabled') || errorMessage.includes('Signups not allowed')) {
                errorMessage = t('auth.errors.anonymous_disabled', 'El registro está deshabilitado temporalmente.');
            }
            ...
            <div className="max-w-sm w-full space-y-10 animate-fade-in">
                ...
                <div className="text-center space-y-2">
                    <h1 className="text-2xl font-serif font-normal text-[#1A1A1A]">
                        {t('signup.title', 'Crear cuenta')}
                    </h1>
                    <p className="text-sm text-[#6B6B6B] font-light">
                        {t('signup.subtitle', 'Empieza tu viaje con SARTORIA')}
                    </p>
                </div>
                ...
                >
                    ...
                    {t('signup.google', 'Registrarse con Google')}
                </button>
                ...
                <p className="text-center text-sm text-[#6B6B6B] font-light">
                    {t('signup.has_account', '¿Ya tienes cuenta?')}{' '}
                    <Link to="/login" className="text-[#1A1A1A] border-b border-[#1A1A1A] hover:opacity-70 transition-opacity">
                        Iniciar sesión
                    </Link>
                </p>
            </div>
